//$ Id: win.cpp 1.02 2006-05-24 14:34:50 +8:00 $

/*
 * PAL(WIN95) Compress Library
 * 
...
*/
/*
PAL95编码 & 解码程序 Version 1.0, Copyright by Lou Yihua
...
static Tree build_tree(void)
{
	...
	TreeNode **list;
	TreeNode *node;
	tree.list = list = new TreeNode * [321];
	tree.node = node = new TreeNode [641];
...
PalLibrary::DataBuffer PalLibrary::DecodeWin(const void *Source)
{
	int len = 0, ptr = 0;
	unsigned int max;
	...
	DataBuffer buf;
	tree = build_tree();
	buf.length = max = *((unsigned int *)src); src += 4;
	buf.data = dest = new unsigned char [max];
...
PalLibrary::DataBuffer PalLibrary::EncodeWin(const void *Source, unsigned int SourceLength)
{
	int match, match_len, len, ptr, pos, table, top, srclen = SourceLength;
	...
	TreeNode *node;
	DataBuffer buf;
	buf.data = dest = new unsigned char [SourceLength];

	*((int *)dest) = srclen; dest += 4;
	pre = prev = _prev;
